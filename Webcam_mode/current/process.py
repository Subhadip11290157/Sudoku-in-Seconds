...
def find_contours(img, original):
    ...
    if polygon is not None:
        ...
        top_left = process_helpers.find_extreme_corners(
            polygon, min, np.add)  # has smallest (x + y) value
        top_right = process_helpers.find_extreme_corners(
            polygon, max, np.subtract)  # has largest (x - y) value
        bot_left = process_helpers.find_extreme_corners(
            polygon, min, np.subtract)  # has smallest (x - y) value
        bot_right = process_helpers.find_extreme_corners(
            polygon, max, np.add)  # has largest (x + y) value
        ...
        [process_helpers.draw_extreme_corners(x, original) for x in [
...
def warp_image(corners, original):
    ...
    mapping = np.array([[0, 0], [width - 1, 0], [width - 1,
                       width - 1], [0, width - 1]], dtype='float32')
...
def recognize_digits(squares_processed, model):
    ...
    all_preds = list(map(np.argmax, model(
        tf.convert_to_tensor(formatted_squares))))
    ...
    for j in range(9):
        for i in range(9):
            if type(squares_processed[index]) == int:
                ...
                text_origin = (center[0] - text_size[0] //
                               2, center[1] + text_size[1] // 2)

                cv2.putText(warped_img, str(solved_puzzle[index]),
                            text_origin, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)  # BGR
            index += 1
...
def unwarp_image(img_src, img_dest, pts, time):
    ...
    warped = cv2.warpPerspective(
        img_src, h, (img_dest.shape[1], img_dest.shape[0]))
    ...
    cv2.putText(dst_img, time, (dst_img_width - 250, 40),
