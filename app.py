# https://www.youtube.com/watch?v=O6WfZRyatcY
import time as t
...
while True:
    ...
    if time_elapsed > 1. / frame_rate:
        ...
        if corners:
            ...
            squares_guesses = process.recognize_digits(
                squares_processed, my_model)
            ...
            if squares_guesses in seen:
                process.draw_digits_on_warped(
                    warped, seen[squares_guesses][0], squares_processed)
                img_result = process.unwarp_image(
                    warped, img_result, corners, seen[squares_guesses][1])

            else:
                solved_puzzle, time = sudoku.solve_wrapper(squares_guesses)
                if solved_puzzle is not None:
                    process.draw_digits_on_warped(
                        warped, solved_puzzle, squares_processed)
                    img_result = process.unwarp_image(
                        warped, img_result, corners, time)
